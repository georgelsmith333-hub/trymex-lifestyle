...
export function GarmentSVG({
  ...
  return (
    <>
      ...
      {/* ── Garment curvature & lighting overlays ─────────────────────────
          Applied to all products — radial vignette darkens edges to simulate
          the fabric wrapping around the body (Printful / Teespring style).
          Mug & bottle also get a strong cylindrical body gradient so the
          product reads as a 3D cylinder even before any design is placed.
      ──────────────────────────────────────────────────────────────────── */}
      <defs>
        {/* Radial edge vignette — stronger than before to add depth */}
        <radialGradient id={`vign-${filterId}`} cx="50%" cy="50%" r="58%"
          gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="1000" y2="1000">
          <stop offset="0%"   stopColor="rgba(0,0,0,0)" />
          <stop offset="55%"  stopColor="rgba(0,0,0,0.02)" />
          <stop offset="80%"  stopColor="rgba(0,0,0,0.08)" />
          <stop offset="100%" stopColor="rgba(0,0,0,0.22)" />
        </radialGradient>
        {/* Top-shoulder / cap highlight — studio key-light simulation */}
        <radialGradient id={`hi-${filterId}`} cx="50%" cy="18%" r="40%" gradientUnits="userSpaceOnUse">
          <stop offset="0%"   stopColor="rgba(255,255,255,0.13)" />
          <stop offset="60%"  stopColor="rgba(255,255,255,0.04)" />
          <stop offset="100%" stopColor="rgba(255,255,255,0)" />
        </radialGradient>
        {/* Mug/bottle cylindrical body — left-edge shadow */}
        ...
        {/* Mug/bottle cylindrical body — right-edge shadow */}
        ...
        {/* Mug/bottle cylindrical body — centre specular highlight */}
      ...
      {/* Curvature shadow + highlight overlay — draws on top of garment photo */}
      ...
      {/* Cylindrical body overlays for mug & water bottle — runs directly on
          the garment photo so the product looks 3D before any design is added. */}
      {(product.category === "mug" || product.category === "waterbottle") && (
        <>
