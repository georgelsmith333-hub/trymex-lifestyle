...
/** Custom hex/colour picker in the Design Studio — enabled so customers
 *  can pick any garment colour beyond the curated preset swatches. */
const STUDIO_CUSTOM_COLOR_ENABLED = true;
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {/* ── Fabric texture blend overlay ─────────────────────────────
                      Composites the garment photo OVER the design layers using SVG
                      multiply blend mode.  Where the garment is pure white the
                      multiply result equals the design pixel unchanged.  Where
                      the fabric has darker tone from studio shadow, cloth weave,
                      or realistic wrinkles the design is proportionally darkened —
                      producing authentic "ink sunk into cotton" depth identical to
                      how Printful / Printify / SPOD render their live mockups.

                      We run this as three sub-passes (mirroring the canvas
                      composer) — multiply 50% + screen 10% + overlay 14%:
                        • multiply darkens design pixels proportionally to garment tonal value
                        • screen lifts bright studio-highlight areas back to near-white
                        • overlay micro-boosts mid-tone contrast for tactile grain feel
                  ─────────────────────────────────────────────────────────────── */}
                  {(() => {
                    const hasVisibleLayer = layersRender.some(
                      ({ layer: l }) => l.visible && (l.face ?? "front") === activeFace
                    );
                    if (!hasVisibleLayer) return null;
                    const base = BASE_BY_CATEGORY[selectedProduct.category];
                    const texSrc = base
                      ? (activeFace === "back" ? (base.back ?? base.front) : base.front)
                      : (activeFace === "back"
                          ? (displayProduct.backSrc ?? displayProduct.frontSrc)
                          : displayProduct.frontSrc);
                    return (
                      <g clipPath="url(#design-clip)" pointerEvents="none">
                        {/* Pass 1 — multiply: fabric tonal depth */}
                        <image href={texSrc} x={0} y={0} width={1000} height={1000}
                          preserveAspectRatio="xMidYMid meet"
                          style={{ mixBlendMode: "multiply" as const, opacity: 0.50 }}
                        />
                        {/* Pass 2 — screen: restore highlight bloom */}
                        <image href={texSrc} x={0} y={0} width={1000} height={1000}
                          preserveAspectRatio="xMidYMid meet"
                          style={{ mixBlendMode: "screen" as const, opacity: 0.10 }}
                        />
                        {/* Pass 3 — overlay: micro-contrast grain pop */}
                        <image href={texSrc} x={0} y={0} width={1000} height={1000}
                          preserveAspectRatio="xMidYMid meet"
                          style={{ mixBlendMode: "overlay" as const, opacity: 0.14 }}
                        />
                      </g>
                    );
                  })()}

                  {/* Cylindrical surface overlay — additional L/R edge darkening and centre
                      specular highlight applied over the texture blend for mug & water bottle.
                      This stacks on top of the fabric texture pass above to give full
                      360° cylinder wrap realism with correct depth falloff on both edges. */}
                  {(selectedProduct.category === "mug" || selectedProduct.category === "waterbottle") && (
                    <g clipPath="url(#design-clip)" pointerEvents="none">
                ...
                {layers.length === 0 && (
                  ...
                  >
                    <motion.div
                      className="text-center px-6 py-6 rounded-2xl transition-all"
                      whileHover={{ scale: 1.02 }}
                      style={{ background: "rgba(255,255,255,0.94)", border: "2px dashed rgba(232,93,4,0.35)", backdropFilter: "blur(4px)" }}
                    >
                      <div
                      ...
                      <p className="font-black text-gray-800 text-sm mb-1">Tap to upload your image</p>
                      <p className="text-[11px] text-gray-500 mb-4">or drag & drop · JPG, PNG, WebP</p>
                      <div className="flex items-center gap-2 justify-center" onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={(e) => { e.stopPropagation(); setActiveTab("text"); }}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border border-gray-200 bg-white hover:border-orange-400 hover:text-orange-600 transition-colors"
                        >
                        ...
                        <button
                          ...
                          style={{ background: "linear-gradient(135deg,#fff4ee,#ffe8d4)", borderColor: "rgba(232,93,4,0.3)", color: "#E85D04" }}
                        >
                          <Wand2 className="w-3 h-3" /> AI Art
                        </button>
                        <button
                          onClick={(e) => { e.stopPropagation(); setActiveTab("templates"); }}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border border-gray-200 bg-white hover:border-orange-400 hover:text-orange-600 transition-colors"
                        >
